...
с параметрами, соответствующими выбранному ОФД. Обмен включает:
    ...
    - Передачу уведомлений о реализации маркированного товара (***находится в тестировании***);
    - Обновление ключей проверки кодов маркировки (***находится в тестировании***).

6. Открытие и закрытие смены и закрытие архива ФН текущей датой или датой последнего документа ФН.

7. Чтение выгрузок архивов ФН в форматах `.fnc` и `.fsd`.
...
ФНС России «Форматы фискальных документов, обязательные к использованию» для версии ФФД 1.05 (***находится в тестировании***).
...
### Сформировать выгрузку архива ФН
...
- всех доступных тегов, в том числе – неизвестных модулю, в представлении hex (*вариант «все»*).
...
поддержка реализована, но нуждается в «боевом» тесте***. Настройки подключения определяются автоматически:
...
уведомлений о реализации, а также обновление ключей проверки кодов маркировки.
...
В противном случае опция будет доступна только при полном считывании данных (кнопка «Открыть» с активным флажком «и прочитать»).
...
- По нажатию кнопки «***Счётчики***» становится доступным отображение накоплений за весь период работы ФН, рассчитанных самим приложением;
